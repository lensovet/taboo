...
class ContentProviderHTTP extends ContentProvider {
	...
	function do_get_contents ( $title ) {
		...
		if ( $xmlg["useapi"] ) {
			$url = "http://" . $xmlg["site_base_url"] . "/api.php?format=php&action=query&prop=revisions&rvexpandtemplates=0&rvprop=timestamp|user|comment|content&titles=" . urlencode ( $title ) ;
